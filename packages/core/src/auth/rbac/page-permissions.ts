...
export interface PagePermissionConfig {
  ...
  // Any authenticated dashboard user may open this non-platform surface.
...
const PAGE_PERMISSION_MAP: Record<string, PagePermissionConfig> = {
  ...
  // Seller workspace is membership-authorized by its API boundary, not platform RBAC.
  "/admin/vendor-dashboard": { allowAuthenticated: true },

  // Platform vendor administration
  "/admin/vendors": { permission: PERMISSIONS.VENDORS_VIEW },
  "/admin/vendors/new": { permission: PERMISSIONS.VENDORS_CREATE },
...
const DYNAMIC_PAGE_PERMISSIONS: Array<{
  ...
  config: PagePermissionConfig;
    ...
    // Vendors
    { pattern: /^\/admin\/vendors\/[^/]+\/edit$/, config: { permission: PERMISSIONS.VENDORS_EDIT } },
    { pattern: /^\/admin\/vendors\/[^/]+$/, config: { permission: PERMISSIONS.VENDORS_VIEW } },
